...
Use the `setuid` binary `suconnect` to connect to a local service and provide the password of the current level.

The service returns the password for the next level when the correct password is provided.

### 💡 Solution

I used the password obtained from Level 19:

```text
4pIjcunZ0fK2vmp3IwfG8Vf7VhxD6pOA
```

The `suconnect` program is used to connect to the appropriate local port and authenticate using the current password.

After successful authentication, the service returned the password for Level 21.
...
* A SUID program can perform operations with the privileges of its owner.
* Local network services can be used as part of authentication challenges.
* The password from the current level can be used to obtain the password for the next level.

### 📌 Key Takeaway

This level demonstrates how a specially configured SUID program can communicate with a local service to authenticate and retrieve the next password.
...
Find the password for Level 22.

A cron job is configured to execute a script automatically. The objective is to inspect the cron configuration and determine how the password is generated or stored.

### 🔎 Enumeration

I inspected the available cron jobs:

```bash
ls /etc/cron.d/
```

The cron configuration relevant to this level can be inspected using:

```bash
cat /etc/cron.d/cronjob_bandit22
```

The cron job executes a script periodically.

I then examined the corresponding script to understand what it does.

### 💡 Solution

By inspecting the cron configuration and the script executed by it, I was able to identify where the password generated by the cron job was stored.
...
* Cron is used to schedule commands and scripts automatically.
* `/etc/cron.d/` contains system cron job configurations.
* Examining a scheduled script can reveal how files are generated or modified.

### 📌 Key Takeaway

When a challenge involves a cron job, inspect both the **cron configuration** and the **script it executes**.
...
Find the password for Level 23.

The level uses a cron job that runs a script as the `bandit23` user. The script creates a file in `/tmp` containing the password.

### 🔎 Enumeration

I first inspected the cron configuration:
...
This showed that `/usr/bin/cronjob_bandit23.sh` runs every minute as `bandit23`.
...
The script uses the username to generate an MD5 hash.
...
The script therefore copies the password into:
...
* Cron jobs can execute scripts automatically.
* `whoami` returns the current username.
* `md5sum` generates an MD5 hash.
* `cut` can extract specific parts of command output.
* Understanding a script's logic can reveal the location of sensitive data.

### 📌 Key Takeaway

Never just run a script blindly in a security challenge. **Read and understand the script first.**

In this level, understanding how `mytarget` was calculated allowed me to predict the exact filename containing the password.
...
Exploit the cron job running as `bandit24` to execute a script with higher privileges and obtain the password for Level 24.
...
```text
drwxrwxrwx 2 bandit23 bandit23 ...
```

I then copied the script into the directory monitored by the `bandit24` cron job:
...
The cron job periodically executes scripts placed in the appropriate spool directory.

I waited for the cron job to execute the script.

After execution, I checked my temporary directory:

```bash
ls -l /tmp/bandit23
```

The output showed a newly created file:

```text
password
```

The file was owned by `bandit24`, showing that the cron job had executed the script with the privileges of `bandit24`.

I then read the file:

```bash
cat /tmp/bandit23/password
```
...
* Cron jobs can execute scripts automatically.
* File permissions determine who can modify files.
* A script executed by another user can create files owned by that user.
* `/var/spool/` can contain directories used by cron jobs.
* Temporary directories such as `/tmp` are useful for testing and receiving output.

### 📌 Key Takeaway

This level demonstrates the security risk of **privileged cron jobs executing user-controlled scripts**.

If a higher-privileged process executes a script that an unprivileged user can modify, it can potentially be abused to perform actions with higher privileges.
...
Find the password for Level 25.

The password obtained for this level is:
...
After completing the Level 24 challenge and obtaining the required credentials, I used the password to access the next level.
...
* Each Bandit level builds on concepts introduced in previous levels.
* Correctly preserving and using passwords is important when moving between levels.
* Linux permissions, cron jobs, scripts, and authentication mechanisms are recurring concepts throughout the Bandit challenges.

### 📌 Key Takeaway

By Level 25, the challenges increasingly combine multiple Linux concepts rather than relying on a single command.
